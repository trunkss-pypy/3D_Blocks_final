...
Karena proyek ini statis, jalankan server HTTP dari direktori proyek. Kamera browser memerlukan konteks aman, sehingga Netlify Dev direkomendasikan:

```bash
netlify dev --port 8889
```

Buka `http://localhost:8889`. Jangan membuka `index.html` langsung melalui protokol `file://`, karena kebijakan browser dapat memblokir kamera dan aset CDN.
...
- `netlify.toml` — konfigurasi publish serta header keamanan dasar
